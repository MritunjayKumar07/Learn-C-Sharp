...
- .NET is a Platform Independent & Language Independent.
...
## Beginner Level
1. What is C#?:
    - C#.NET (`C-Sharp`) bellong from C++ background or you say (C# is a extension of C++ language). But C++ only use for build desktop application and (`C-Sharp`) use to build any type of application. And `C-Sharp` also a most powerfull language as compair to another .NET languagaee.
    - The `#` or `Sharp` is taken from musical note.
    - `C-Sharp` is a type-safe, Object Oriented, Platform Independent & Language Independent.
